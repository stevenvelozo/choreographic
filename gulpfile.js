...
() => {
	...
	{
		entries: './source/Choreographic-Browser-Shim.js',
		standalone: 'Fable',
		debug: true
	});

	return tmpBrowserify.bundle()
		.pipe(libVinylSourceStream('choreographic.min.js'))
		.pipe(libVinylBuffer())
...
libGulp.task('debug',
	() => {
		...
		{
			entries: './source/Choreographic-Browser-Shim.js',
			standalone: 'Choreographic',
			debug: true
		});

		return tmpBrowserify.bundle()
			.pipe(libVinylSourceStream('choreographic.js'))
			.pipe(libVinylBuffer())
					.on('error', libGulpUtil.log)
			.pipe(libGulp.dest('./dist/'));
	});
